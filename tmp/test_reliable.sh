#!/usr/bin/env bash
# Exhaustive ISOLATED attribute test - reliable version.
# - restore product 64 baseline before each test (isolation)
# - ?nocache=1 bypasses page_fw full-page cache (fresh DB render every fetch)
# - marker = cart.add('$PID'  (prefix; add button always renders via fallback)
PID=64
BASE="http://localhost:8081"
HOME_URL="$BASE/?nocache=1"
SHOP_URL="$BASE/index.php?route=product/category&path=59&nocache=1"
...
check_page() {
  ...
  if echo "$body" | grep -qiE 'Fatal error|Parse error|Notice: |Warning: |Undefined (variable|index|offset)|Uncaught'; then
    echo "PHP-ERR:$(echo "$body" | grep -oiE '(Fatal error[^<]{0,50}|Notice: [^<]{0,50}|Warning: [^<]{0,50}|Uncaught[^<]{0,50})' | head -1)"; return 1
  fi
...
T() {
  ...
  if [ -z "$r1" ] && [ -z "$r2" ]; then echo "PASS  $name"; PASS=$((PASS+1))
  else echo "FAIL  $name :: $r1 $r2"; FAIL=$((FAIL+1)); FAILED_NAMES="$FAILED_NAMES|$name"; fi
}

# EAV tag test: INSERT a product_to_custom_tag row for product 64, verify card still shows
TE() {
  local name="$1"; local tagid="$2"; local val="$3"
  restore_p64
  echo "DELETE FROM oc_product_to_custom_tag WHERE product_id=$PID AND tag_id=$tagid; INSERT INTO oc_product_to_custom_tag (product_id,tag_id,value) VALUES ($PID,$tagid,'$val')" | sql
  local r1 r2=""
  r1=$(check_page "$HOME_URL" "present"); [ $? -ne 0 ] && r1="HOME:$r1" || r1=""
  r2=$(check_page "$SHOP_URL" "present"); [ $? -ne 0 ] && r2="SHOP:$r2" || r2=""
  if [ -z "$r1" ] && [ -z "$r2" ]; then echo "PASS  $name"; PASS=$((PASS+1))
  else echo "FAIL  $name :: $r1 $r2"; FAIL=$((FAIL+1)); FAILED_NAMES="$FAILED_NAMES|$name"; fi
}

echo "===== ISOLATED TESTS (nocache, corrected marker) ====="
...
echo "--- EAV tag value tests (card must still display) ---"
TE "eav color red"    38 "red"
TE "eav color empty"  38 ""
TE "eav color special" 38 "re'd & <blue>"
TE "eav dd img"       46 "catalog/demo/product/product-2.png"
TE "eav dd badpath"   46 "nonexistent/xyz.png"
TE "eav dd empty"     46 ""
TE "eav 长 number"    33 "999"
TE "eav 长 empty"     33 ""
TE "eav 长 nonnum"    33 "abc"
