...
class ControllerAccountWishList extends Controller {
	public function index() {
		...
		if (isset($this->request->get['remove'])) {
			// Remove Wishlist (DB for logged-in customers, session for guests)
			if ($this->customer->isLogged()) {
				$this->model_account_wishlist->deleteWishlist($this->request->get['remove']);
			} else {
				if (isset($this->session->data['wishlist']) && is_array($this->session->data['wishlist'])) {
					$this->session->data['wishlist'] = array_values(array_diff($this->session->data['wishlist'], array((int)$this->request->get['remove'])));
				}
			}
		...
		$this->document->setTitle('Wishlist - ' . $this->config->get('config_name'));
		...
		// Logged-in: read from DB. Guest: read from session (migrated to DB on login).
		if ($this->customer->isLogged()) {
		...
		foreach ($results as $result) {
			...
			if ($product_info) {
				...
				// Product no longer exists - clean up the stale entry
				if ($this->customer->isLogged()) {
					$this->model_account_wishlist->deleteWishlist($result['product_id']);
				} else {
					if (isset($this->session->data['wishlist']) && is_array($this->session->data['wishlist'])) {
						$this->session->data['wishlist'] = array_values(array_diff($this->session->data['wishlist'], array((int)$result['product_id'])));
					}
				}
			}
		}

		$data['shop_url'] = $this->url->link('product/category', 'path=59');
	 ...
	 * Logged-in -> DB (customer_wishlist). Guest -> session.
	 * Returns JSON: { success, in_wishlist, action: added|removed, total, product_name }
	 */
	...
	/**
	 * Return the user's wishlist product IDs (DB or session) as JSON.
	 * Used by the product-card hearts to render their initial filled state.
	 */
	public function ids() {
		$json = array('ids' => array());

		if ($this->customer->isLogged()) {
			$this->load->model('account/wishlist');

			foreach ($this->model_account_wishlist->getWishlist() as $row) {
				$json['ids'][] = (int)$row['product_id'];
			}
		} else {
			if (isset($this->session->data['wishlist']) && is_array($this->session->data['wishlist'])) {
				foreach ($this->session->data['wishlist'] as $product_id) {
					$json['ids'][] = (int)$product_id;
				}
			}
		}
