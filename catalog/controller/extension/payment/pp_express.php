...
class ControllerExtensionPaymentPPExpress extends Controller {
	...
	public function expressComplete() {
	    $log = new Log('pp_express_checkout_complete.log');
	    $log->write('into successful.');
		$this->load->language('extension/payment/pp_express');
		$redirect = '';
		...
		if ($redirect == '') {
			...
			if ($result['ACK'] == 'Success') {
				...
				$log->write('order history status id: ' . $order_status_id);
		    ...
		    $log->write('redirect to: ' . $redirect);
	...
	public function checkout() {
		...
		$result = \Paypal\Paypal::getSingleton()->confirmExpress();
		$token = array_get($result, 'token', '');

		$this->session->data['paypal']['token'] = $token;

		if ($this->config->get('payment_pp_express_test') == 1) {
			header('Location: https://www.sandbox.paypal.com/cgi-bin/webscr?cmd=_express-checkout&token=' . $token . '&useraction=commit');
		} else {
			header('Location: https://www.paypal.com/cgi-bin/webscr?cmd=_express-checkout&token=' . $token . '&useraction=commit');
		}
	}
